...
bool getNVMeInfobyBusID(int busID, int address, int channel, phosphor::nvme::Nvme::NVMeData& nvmeData)
{
...
We can tell what kind of device is connected by reading driverPresent and driverIfdet.
...
void Nvme::read()
{
    ...
    for (auto config : configs)
    {
        ...
        if (getNVMePresent(a) == 1)
        {
            ...
            auto success = getNVMeInfobyBusID(config.busID, config.address, config.channel, nvmeData);
